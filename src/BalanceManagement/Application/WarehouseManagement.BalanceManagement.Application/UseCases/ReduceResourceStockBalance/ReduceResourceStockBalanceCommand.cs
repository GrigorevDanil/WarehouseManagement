...
public record ReduceResourceStockBalanceCommand(Guid ResourceId, Guid UnitId, int SubtractedResourceStock) : ICommand
{
    public static ReduceResourceStockBalanceCommand Create(Guid resourceId,Guid unitId, ReduceResourceStockBalanceRequest request) => 
        new(resourceId, unitId, request.SubtractedResourceStock);
}